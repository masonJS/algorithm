/**
   ...
   신호를 송/수신하는 장치를 설치했습니다.
   발사한 신호는 신호를 보낸 탑보다높은 탑에서만 수신합니다.
   또한, 한 번 수신된 신호는 다른 탑으로 송신되지 않습니다.

   예를 들어 높이가 6, 9, 5, 7, 4인 다섯 탑이 으로
   동시에 레이저 신호를 발사합니다.
...
function solution(heights) {
 return heights.map((height, index) => {
   ...
   const findIndex = list.findIndex(h => h > height)
   return findIndex > 0 ? list.length - findIndex : 0
 })
}
